...
func diffMap(expect, actual value.IPersistentMap) value.Associative {
	...
	for entrySeq := value.Seq(expect); entrySeq != nil; entrySeq = value.Next(entrySeq) {
		...
		// ignore meta for now
		if value.Equal(entry.Key(), kw("meta")) {
			continue
		}
