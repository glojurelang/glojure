...
func (env *environment) EvalASTMaybeHostForm(n *ast.Node) (interface{}, error) {
	...
	// Go package export or JVM static member (short or fully-qualified
	// class name). Shared with AOT code so both paths agree.
	if v, ok := pkgmap.LookupHostMember(hostFormNode.Class, field.Name()); ok {
		return v, nil
	}
