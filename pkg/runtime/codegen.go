...
type Generator struct {
	...
	// Module name for generating import paths
	moduleName string
}
...
func (g *Generator) Generate(ns *lang.Namespace) error {
	...
	requiredNamespaces := make(map[string]bool) // Track namespaces that are referenced
	...
	for seq := mappings.Seq(); seq != nil; seq = seq.Next() {
		...
		if !(vr.Namespace() == ns && lang.Equals(vr.Symbol(), name)) {
			// This is a var from another namespace (e.g., `:refer`red)
			if vr.Namespace() != nil {
				foreignNS := vr.Namespace().Name().String()
				// Skip clojure.core as it's handled separately
				if foreignNS != "clojure.core" && foreignNS != ns.Name().String() {
					requiredNamespaces[foreignNS] = true
				}
			}
	...
	// Add imports for required namespaces
	for foreignNS := range requiredNamespaces {
		nsPath := g.nsToGoImportPath(foreignNS)
		g.addImportUnderscore(nsPath)
	}
	...
	{
		// Determine namespace format based on GLJ_USE_AOT
		// When GLJ_USE_AOT is false (stdlib generation), use `/` for namespace sep
		// else use dots
		useAOT := GetUseAOT()
		namespaceName := ns.Name().String()
		if !useAOT {
			// Convert dots to slashes for non-AOT (stdlib) mode
			namespaceName = nsToPath(namespaceName)
		}

		initBuf.WriteString(`func init() {
`)
		// Load required namespaces before registering this one
		// Sort for deterministic output
		var sortedRequiredNS []string
		for foreignNS := range requiredNamespaces {
			sortedRequiredNS = append(sortedRequiredNS, foreignNS)
		}
		sort.Strings(sortedRequiredNS)
		for _, foreignNS := range sortedRequiredNS {
			// Use dots format for GetNSLoader calls (always)
			initBuf.WriteString(fmt.Sprintf("\truntime.GetNSLoader(%q)()\n", foreignNS))
		}
		initBuf.WriteString(`runtime.RegisterNSLoader(` + fmt.Sprintf("%q", namespaceName) + `, LoadNS)
}
...
func (g *Generator) addImportUnderscore(pkg string) {
	g.imports[pkg] = "_"
}

func (g *Generator) SetModuleName(moduleName string) {
...
func (g *Generator) nsToGoImportPath(ns string) string {
	// Convert namespace to Go import path
	// e.g., "ys.v0" -> "<module>/ys"
	// The module name is extracted from the input filename or GLJ_AOT_MODULE env var
	parts := strings.Split(ns, ".")
	if len(parts) > 1 {
		// Return module-relative path (last part is package name)
		basePath := strings.Join(parts[:len(parts)-1], "/")
		return g.moduleName + "/" + basePath
	}
	return g.moduleName + "/" + ns
}
